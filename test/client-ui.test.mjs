...
test('all panels expose persistent collapse controls', async () => {
  ...
  const styles = await readFile(new URL('src/client/styles.mjs', root), 'utf8')
  assert.match(composite, /loadCollapse\(\)/)
  assert.match(composite, /saveCollapse\(next\)/)
  ...
  assert.match(importer, /dsh-ccswitch-import-body/)
  assert.match(importer, /id: "dsh-ccswitch-import-body".*hidden: collapsed/)
  assert.match(reasoning, /withModelToggled/)
  assert.ok(reasoning.includes('onToggleCollapsed?.(route, model.id, !collapsed)'))
  assert.match(reasoning, /dsh-reasoning-collapse/)
  assert.match(reasoning, /aria-controls.*dsh-reasoning-model-body/)
  assert.match(reasoning, /!collapsed && h\("footer"/)
  assert.match(collapse, /COLLAPSE_KEY/)
  assert.match(collapse, /export function loadCollapse/)
  assert.match(collapse, /export function saveCollapse/)
  assert.match(styles, /\.dsh-reasoning-embed__toggle\{[^}]*cursor:pointer/)
  assert.match(styles, /\.dsh-reasoning-collapse,\.dsh-ccswitch-collapse\{/)
  assert.match(styles, /\.dsh-ccswitch-import__header-actions\{/)
})
